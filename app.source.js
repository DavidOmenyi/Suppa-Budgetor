...
const availableActions = [
    ...
    { id: 'summaries', icon: '📁', label: 'Reports', color: '#10b981' }, // ADDED FOR QUICK ACTIONS
    { id: 'help', icon: '📞', label: 'Help', color: 'var(--accent)' }
];
...
window.updateUI = function() {
    try {
        ...
        Object.values(catGroups).sort((a,b) => a.cat.localeCompare(b.cat)).forEach((group, index) => {
            ...
            if (spentFilter === 'ZERO' && visibleActual !== 0) return;
            if (spentFilter === 'NON_ZERO' && visibleActual === 0) return;
            if (spentFilter === 'UNDER_BUDGET' && (visibleBudget === 0 || visibleActual >= visibleBudget)) return;

            tableBudget += visibleBudget; 
            tableSpent += visibleActual;

            let catVariance = visibleBudget - visibleActual; 
            let catIsPositive = catVariance >= 0;
            let catVarPct = visibleBudget ? ((catVariance / visibleBudget) * 100).toFixed(1) : 0;
            let rowId = `cat-group-${index}`;

            if(perfBody) {
                perfBody.innerHTML += `
                    <tr style="cursor: pointer; background: rgba(5,150,105,0.05); transition: 0.2s;" onclick="document.querySelectorAll('.${rowId}').forEach(el => el.classList.toggle('hidden'))">
                        <td style="font-weight: bold; border-left: 3px solid var(--primary);">${window.getIcon(group.cat)} ${group.cat}</td>
                        <td style="color: var(--primary); font-size: 12px; font-weight: bold;">View ▼</td>
                        ...
                        <td style="font-weight: bold;">${visibleBudget.toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                        <td style="font-weight: bold;">${visibleActual.toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                ...
                visibleItems.sort((a,b) => a.name.localeCompare(b.name)).forEach(item => {
                    ...
                    perfBody.innerHTML += `
                        <tr class="${rowId} hidden" style="background: var(--surface-color); border-bottom: 1px dashed var(--border);">
                            ...
                            <td style="font-size: 0.9em;"><a class="ledger-link" onclick="window.openLedger('${group.cat.replace(/'/g, "\\'")}', '${item.name.replace(/'/g, "\\'")}', false)">${dispName}</a></td>
                            <td></td>
...
window.requestPDFPassword = function(reason) {
    return new Promise((resolve) => {
        ...
        if (reason > 1) {
            ...
            title.innerHTML = "Enter Statement Password";
...
// ==========================================
// UNIFIED MASTER BACKUP & RESTORE UTILITIES
// ==========================================

// One-click master export for the entire application state
window.exportMasterBackupJSON = function() {
    const masterPayload = {
        version: "2026.1",
        ...
        customMem: customMem || { Names: [], Expense: [], Savings: [] },
        // 2. Shopping Lists Data
    ...
    window.triggerDownload(
        jsonString, 
        `Suppa_Budgetor_MASTER_BACKUP_${dateStr}.json`, 
        "application/json"
    );
};

// One-click master recovery engine
window.importMasterBackupJSON = function(e) {
    const file = e.target.files[0];
    if (!file) return;

    if (!confirm("WARNING: Restoring this master backup will overwrite your current active data. Do you wish to proceed?")) {
        e.target.value = '';
        return;
    }
    ...
    reader.onload = function(evt) {
        try {
            ...
            if (!backup.transactions && !backup.shoppingLists && !backup.categoryBudgets) {
                throw new Error("Invalid backup file structure.");
            }

            window.saveState(); // Save current state to Undo history just in case

            // 1. Restore Core Financials (with fallbacks)
            transactions = backup.transactions || [];
            categoryBudgets = backup.categoryBudgets || {};
            customMem = backup.customMem || { Names: [], Expense: [], Savings: [] };

            // 2. Restore Shopping Lists
            shoppingLists = backup.shoppingLists || {};

            // 3. Commit everything cleanly to device LocalStorage
            window.saveData();         // Saves transactions, budgets, memory
            window.saveShoppingData(); // Saves shopping lists

            // 4. Hot-reload the entire user interface across tabs
            window.updateDatalists();
            window.updateUI();
            window.updateCharts();
            window.updateInflationChart();
            ...
            alert("🎉 Master Application Restoration Complete! All ledgers, budgets, and shopping planners have been successfully recovered.");

        } catch (err) {
            console.error("Master import failure:", err);
            alert("Agape! Restoration failed. Please ensure you are uploading a valid .json master file generated by Suppa Budgetor.");
        }
...
// Helper extension to support variable mime-types during download triggers
const originalTriggerDownload = window.triggerDownload;
window.triggerDownload = function(content, fileName, contentType = 'text/csv;charset=utf-8;') {
    if (contentType === "application/json") {
        ...
        // Fall back to your existing CSV download trigger mechanism
        if (typeof originalTriggerDownload === "function") {
            originalTriggerDownload(content, fileName);
        } else {
            const blob = new Blob([content], { type: contentType });
            const link = document.createElement("a");
            link.href = URL.createObjectURL(blob);
